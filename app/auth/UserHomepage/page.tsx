          ...
          <div className="p-6 relative z-10 flex flex-col md:flex-row items-center gap-6">
              ...
              <div className="w-24 h-24 rounded-full overflow-hidden border-2 border-amber-700/50 shadow-[0_0_10px_rgba(255,170,0,0.3)]">
                {user.avatar_url ? (
                  ...
                  <div className="w-full h-full bg-amber-900/30 flex items-center justify-center">
                    <span className="text-3xl text-amber-500">{user.username.charAt(0).toUpperCase()}</span>
                  </div>
                )}
              </div>
            </div>
            ...
            <div className="text-center md:text-left flex-1">
              <h1 className="text-2xl md:text-3xl font-serif text-amber-200 mb-2 tracking-wider">
                {user.title && <span className="text-amber-500 mr-2">{user.title}</span>}
                {user.username}
              </h1>
                      ...
                      <div className="mt-4">
                        ...
                        <div className="flex flex-wrap gap-2">
                          {character.traits.map((trait: { name: string; rarity: number }, idx: number) => (
                            <div 
                              key={idx} 
                              className={`text-xs px-2 py-1 rounded-sm border border-amber-900/30 bg-black/30 ${getRarityClass(trait.rarity)}`}
                            >
                              {trait.name}
                            </div>
                          ))}
              ...
              artifacts.map(artifact => (
                ...
                >
                  <div className={`absolute top-0 right-0 w-16 h-16 overflow-hidden ${
                    artifact.rarity >= 90 ? 'bg-purple-500/10' :
                    artifact.rarity >= 75 ? 'bg-amber-500/10' :
                    artifact.rarity >= 50 ? 'bg-cyan-500/10' :
                    'bg-emerald-500/10'
                  }`}>
                    <div className={`absolute top-0 right-0 rotate-45 transform origin-bottom-right w-24 ${
                      artifact.rarity >= 90 ? 'bg-purple-500/30' :
                      artifact.rarity >= 75 ? 'bg-amber-500/30' :
                      artifact.rarity >= 50 ? 'bg-cyan-500/30' :
                      'bg-emerald-500/30'
                    }`}>
                      <div className="py-1 text-center text-xs tracking-wider font-serif text-white/80">
                        {artifact.rarity >= 90 ? 'Legendär' :
                         artifact.rarity >= 75 ? 'Episch' :
                         artifact.rarity >= 50 ? 'Selten' :
                         'Gewöhnlich'}
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="pt-6">
                    <h3 className={`font-serif text-lg mb-2 tracking-wide ${
                      artifact.rarity >= 90 ? 'text-purple-300' :
                      artifact.rarity >= 75 ? 'text-amber-400' :
                      artifact.rarity >= 50 ? 'text-cyan-300' :
                      'text-emerald-400'
                    }`}>
                      {artifact.name}
                    </h3>
                ...
                <div className="space-y-4">
                  <div>
                    <div className="flex justify-between items-center text-sm mb-1">
                      ...
                      <span className="text-amber-300">{user.stats?.quests_completed || 0}</span>
                    </div>
                  ...
                  <div>
                    <div className="flex justify-between items-center text-sm mb-1">
                      ...
                      <span className="text-amber-300">{user.stats?.fame_points || 0}</span>
                    </div>
                    <div className="h-1.5 bg-black/60 rounded-full overflow-hidden">
                      <div 
                  ...
                  <div>
                    <div className="flex justify-between items-center text-sm mb-1">
                      ...
                      <span className="text-amber-300">{artifacts.length}</span>
                    </div>
                    <div className="h-1.5 bg-black/60 rounded-full overflow-hidden">
                      <div 
                  ...
                  <div>
                    <div className="flex justify-between items-center text-sm mb-1">
                      ...
                      <span className="text-amber-300">{user.stats?.wisdom_score || 0}</span>
                    </div>
                    <div className="h-1.5 bg-black/60 rounded-full overflow-hidden">
                      <div 
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    {user.achievements.map((achievement: { icon: string; name: string; description: string }, idx: number) => (
                      <div key={idx} className="border border-amber-900/30 bg-black/30 p-3 rounded-sm flex">
                        ...
                        <div>
                          <div className="text-amber-300 text-sm">{achievement.name}</div>
                          <div className="text-amber-200/60 text-xs mt-1">{achievement.description}</div>
                        </div>
                      </div>
                    ))}
                  </div>
